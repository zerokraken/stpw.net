...
@push('scripts')
    ...
    <Script>
            ...
            $(document).on('keyup change', '.price', function () {
                ...
                var totalItemPrice = 0;
                var inputs_quantity = $(".quantity");resources/views/invoice/edit.blade.php
                var priceInput = $('.price');
                for (var j = 0; j < priceInput.length; j++) {
    ...
    @if (module_is_active('Taskly') )
        <script>
            $(document).on('change', '.item', function() {
                ...
                var el = $(this);resources/views/invoice/edit.blade.php
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response != null) {
                                ...
                                $(".price").change();
                            } else {resources/views/invoice/edit.blade.php
                        },
                    });
                }
                else
    ...
    <script>
        $(document).on('change',"[name='invoice_type_radio']", function() {
                ...
                else if(val == 'project')
                {
                    ...
                    var label  = ` {{ Form::label('project', __('Project'),['class'=>'form-label']) }} {{ Form::select('project',$projects,$invoice->category_id, array('class' => 'form-control','required'=>'required')) }}`
                    $(".invoice_div").append(label);
        ...
        function SectionGet(type = 'product',project_id = "0",title = 'Project'){
            ...
            $.ajax({
                    ...
                    data: {resources/views/invoice/edit.blade.php
...
@section('content')
    <div class="row">
        {{ Form::model($invoice, array('route' => array('invoice.update', $invoice->id), 'method' => 'PUT','class'=>'w-100', 'enctype' => 'multipart/form-data')) }}
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group" id="customer-box">
                                ...
                                {{ Form::select('customer_id', $customers,!empty($invoice->user_id) ? $invoice->user_id : null, array('class' => 'form-control ','id'=>'customer','data-url'=>route('invoice.customer'),'required'=>'required','placeholder' =>'Select Customer')) }}
                            </div>
                            ...
                            <div class="row">
                                        ...
                                        <div class="form-group">
                                            <div class="d-flex radio-check">
                                                <div class="form-check form-check-inline form-group col-md-3">
                                                    <input type="radio" id="product" value="product" name="invoice_type_radio" class="form-check-input code" {{ ($invoice->invoice_module =='account') ? 'checked="checked"' :''}}>
                                                    <label class="custom-control-label " for="product">{{__('Item Wise')}}</label>
                                                </div>
                                                <div class="form-check form-check-inline form-group col-md-3">
                                                    <input type="radio" id="project1" value="project" name="invoice_type_radio" class="form-check-input code" {{ ($invoice->invoice_module =='taskly') ? 'checked="checked"' :''}}>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-icon-user">
                                            {{Form::date('due_date',null,array('class'=>'form-control ','required'=>'required','placeholder'=>'Select Customer'))}}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group invoice_div">
                                       ...
                                       @elseif (module_is_active('Taskly'))
                                            ...
                                            {{ Form::select('project',$projects,$invoice->category_id, array('class' => 'form-control','required'=>'required')) }}
                                       @endif
                                    </div>
                                </div>
                                @if (module_is_active('Taskly'))
                                    <div class="col-md-6 tax_project_div {{ (module_is_active('Account') ? "d-none" : '')}}">
                                        <div class="form-group">
                                            ...
                                            {{ Form::select('tax_project[]',$taxs,!empty($invoice->items->first()->tax) ? explode(",",$invoice->items->first()->tax) :null, array('class' => 'form-control get_tax multi-select choices','multiple'=>'multiple','id' => 'tax_project','placeholder' => 'Select Tax')) }}
                                        </div>
                                    </div>
                                @endif
                                @if(module_is_active('CustomField') && !$customFields->isEmpty())
                                    <div class="col-md-12">
                                        <div class="tab-pane fade show" id="tab-2" role="tabpanel">
                                            @include('customfield::formBuilder',['fildedata' => !empty($invoice->customField) ? $invoice->customField : ''])
                                        </div>
                                    </div>
                                @endif
